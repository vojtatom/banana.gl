...
async def create_access_token(data: dict, expires_delta: Optional[timedelta] = None):
    ...
    encoded_jwt = jwt.encode(to_encode, mws.security.secret, algorithm=ALGORITHM)
    return encoded_jwt
...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    try:
        payload = jwt.decode(token, mws.security.secret, algorithms=[ALGORITHM])
        ...
        raise credentials_exception
    user = mws.security.get_user(token_data.username)
    if user is None:
        raise credentials_exception
    return user
